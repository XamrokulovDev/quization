...
const Html = () => {
    const { html } = useContext(userContext);
    const [currentIndex, setCurrentIndex] = useState(0);
    const [answers, setAnswers] = useState([]);
    const [showResult, setShowResult] = useState(false);

    const handleAnswerClick = (answer) => {
        const currentQuestion = html[currentIndex];
    ...
    const getScorePercentage = () => {
        ...
        const totalQuestions = html.length;
    ...
    if (!html || html.length === 0) return <p>No questions available.</p>;
        ...
        return (
            <div className="w-[85vw] h-[100vh] flex items-center justify-center">
              <div className="w-[350px] max-md:w-full backdrop-blur-lg bg-[#eeeeee9b] overflow-hidden rounded-2xl relative p-5">
                ...
                <p className='text-gray-700 p-1 text-md font-medium'>To'g'ri javoblar soni: {getCorrectAnswersCount()} / 30</p>
